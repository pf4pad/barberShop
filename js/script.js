const API__URL = "https://quasar-wandering-sound.glitch.me";
...
const renderSpec = (wrapper, data) => {
  const labels = data.map((item) => {
    ...
    label.innerHTML = `
      ...
      <span class="radio__label radio__label_spec" style="--bg-image: url(${API__URL}/${item.img})">${item.name}</span>
    `;
    return label;
  });
  wrapper.append(...labels);
...
const initReserve = () => {
  ...
  reserveForm.addEventListener("change", async (e) => {
    ...
    if (target.name === "service") {
      ...
      const response = await fetch(`${API__URL}/api?service=${target.value}`);
    ...
    if (target.name === "spec") {
      ...
      const response = await fetch(`${API__URL}/api?spec=${target.value}`);
      data = await response.json();
    ...
    if (target.name === "month") {
      ...
      const response = await fetch(
        `${API__URL}/api?spec=${reserveForm.spec.value}&month=${reserveForm.month.value}`
      );
      data = await response.json();
    ...
    if (target.name === "day") {
      ...
      const response = await fetch(
        `${API__URL}/api?spec=${reserveForm.spec.value}&month=${reserveForm.month.value}&day=${target.value}`
      );
      data = await response.json();
  ...
  reserveForm.addEventListener("submit", async (e) => {
    ...
    const response = await fetch(`${API__URL}/api/order`, {
      method: "post",
      body: json,
    });
